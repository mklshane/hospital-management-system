...
const DoctorProfile = () => {
  ...
  // Initialize CRUD operations for doctor updates
  const {
    update,
    remove,
    loading: crudLoading,
  } = useCrudOperations("Doctor", () => {});
  ...
  useEffect(() => {
    if (user) {
      ...
      setScheduleTime(user.schedule_time || []);
      setOriginalData({ ...userData, schedule_time: user.schedule_time || [] });
    }
  }, [user]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const success = await update(user._id, updateData, `/doctor/${user._id}`);

      if (success) {
        setOriginalData({ ...formData, schedule_time: scheduleTime });
        setIsEditing(false);
      } else {
      }
    } catch (error) {
  ...
  const handleCancel = () => {
    ...
    setScheduleTime(originalData.schedule_time || []);
    setIsEditing(false);
  ...
  const handleDeleteAccount = async () => {
    ...
    setLoading(true);
    try {
      const success = await remove(user._id, `/doctor/${user._id}`);
  ...
  return (
    <>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-600 dark:text-blue-400 mt-1">
                  Your professional information is encrypted and stored
                  securely. We never share your data with third parties without
                  your consent.
                </p>
              </div>
            </div>
      ...
      <DeleteModal
        ...
        description="This will permanently delete your doctor account, appointments, and all associated data. This action cannot be undone."
