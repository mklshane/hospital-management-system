...
const DoctorDetailsModal = ({ isOpen, onClose, doctor, onDelete, refetch }) => {
  ...
  const { update, actionLoading } = useCrudOperations("Doctor", refetch);
  ...
  const handleSave = async () => {
    ...
    const success = await update(doctor._id, payload, "/doctor");
    if (success) {
      setIsEditing(false);
      onClose();
    }
                  ...
                  {isEditing ? (
                    <>
                      <button
                        ...
                        className="flex items-center gap-2 px-5 py-2.20 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-all"
                      >
                      ...
                      <button
                        onClick={handleSave}
                        disabled={isLoading || scheduleTime.length === 0}
